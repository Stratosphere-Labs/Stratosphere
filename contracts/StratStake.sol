...
contract StratStake is Ownable, ReentrancyGuard {
    ...
    // blocked addresses
    mapping(address => bool) private _isExcludedFromStaking;
    ...
    constructor() {
        STAKING_DEPLOYER = msg.sender;
    }

    function excludeFromStaking(address user) external onlyOwner {
        _isExcludedFromStaking[user] = true;
    }

    function includeFromStaking(address user) external onlyOwner {
        _isExcludedFromStaking[user] = false;
    }
    ...
    function initialize(
        ...
        require(msg.sender == STAKING_DEPLOYER, "Not deployer");
    ...
    function deposit(uint256 _amount) external nonReentrant {
        require(
            !_isExcludedFromStaking[msg.sender],
            "You are excluded from staking"
        );
